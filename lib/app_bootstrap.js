function ensureWindowsStartupRegistration({ app, processObj, enabled = false, logger = console }) {
  ...
  try {
    ...
    if (Boolean(current?.openAtLogin) !== Boolean(enabled)) {
      logger.warn(`Failed to ${enabled ? 'enable' : 'disable'} startup registration for this app.`);
    }
  } catch (e) {
