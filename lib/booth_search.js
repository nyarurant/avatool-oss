...
async function fetchBoothItemDetail(itemId, cookieHeader = '') {
  const id = String(itemId || '').trim();
  if (!id) return { error: 'itemId_required' };
  const url = `https://booth.pm/ja/items/${id}.json`;
  const headers = { 'User-Agent': UA, 'Accept': 'application/json' };
  // With a logged-in cookie, BOOTH annotates each variation's own purchase
  // status (e.g. "already_bought_digital" vs "addable_to_cart") — the ground
  // truth for whether *that specific* variation was bought, as opposed to just
  // "is this itemId anywhere in the local library" which conflates bundles
  // (e.g. a "Full Pack" that unlocks every other variation's files) with
  // genuinely separate, still-unpurchased variations.
  ...
  return {
    ...
    variations: Array.isArray(d.variations)
      ? d.variations.map(v => ({
        ...
        name: v.name || '',
        price: v.price ?? null,
        inStock: !v.is_sold_out,
        alreadyBought: v.status === 'already_bought_digital',
      }))
      : [],
