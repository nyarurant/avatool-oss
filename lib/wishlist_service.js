...
function createWishlistService({
  ...
  async function addItemToAvatoolWishListName(itemId) {
    try {
      ...
      if (!avatoolCode) return { ok: false, error: 'avatool_wishlist_code_unavailable' };
      ...
      if (currentCodes.includes(avatoolCode)) return { ok: true, changed: false };
  ...
  async function resolveWishlistCandidate(rawInput, { syncBooth = true } = {}) {
    await ensureClientReady();
    ...
    // Existing UI callers use the default best-effort synchronization. MCP
    // callers set syncBooth:false and await the result after local persistence.
    if (syncBooth) addItemToAvatoolWishListName(itemId).catch(() => {});
