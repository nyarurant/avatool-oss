...
function createMetaManager(deps) {
  ...
  function toAssetMap(data) {
    ...
    for (const item of (data || [])) {
      ...
      const freeAccessState = ['available', 'ended', 'unknown'].includes(String(item?.freeAccessState || ''))
        ? String(item.freeAccessState)
        : (item?.isFreeDownload ? 'unknown' : '');
      if (freeAccessState === 'ended' && !isDownloaded) continue;
      ...
      map[item.itemId] = {
        ...
        files: (freeAccessState === 'ended' ? [] : (item.downloadLinks || [])).map(dl => ({
          downloadableId: dl.downloadableId,
          fileName: dl.fileName,
        })),
        versionHistory: item.versionHistory || [],
        latestVersion: item.latestVersion || null,
        hasUpdate: freeAccessState === 'ended' ? false : Boolean(item.hasUpdate),
  ...
  function lastKnownDownloadLinks(item) {
    if (Array.isArray(item?.downloadLinks) && item.downloadLinks.length > 0) {
      return item.downloadLinks.map((link) => ({ ...link }));
    }
    const history = Array.isArray(item?.versionHistory) ? item.versionHistory : [];
    for (let index = history.length - 1; index >= 0; index -= 1) {
      const links = history[index]?.downloadLinks;
      if (Array.isArray(links) && links.length > 0) return links.map((link) => ({ ...link }));
    }
    return [];
  }

  function repairEndedFreeDownload(item, accessState = item?.freeAccessState) {
    if (!item?.isFreeDownload || accessState !== 'ended') return { ...item };
    const downloadLinks = lastKnownDownloadLinks(item);
    const versionHistory = (Array.isArray(item.versionHistory) ? item.versionHistory : [])
      .filter((entry) => Array.isArray(entry?.downloadLinks) && entry.downloadLinks.length > 0)
      .map((entry) => ({ ...entry, downloadLinks: entry.downloadLinks.map((link) => ({ ...link })) }));
    const lastUsableVersion = versionHistory.at(-1) || null;
    const latestVersion = downloadLinks.length > 0
      ? {
          detectedAt: lastUsableVersion?.detectedAt || item?.latestVersion?.detectedAt || item?.lastChecked || new Date().toISOString(),
          filesHash: generateFilesHash(downloadLinks),
          filesHashStable: generateFilesStableHash(downloadLinks),
        }
  ...
  function applyVersionTrackingKeepingManual(existingMeta, latestMeta, detectedAt = new Date().toISOString()) {
    const latestById = new Map((latestMeta || []).map((item) => [String(item?.itemId || ''), item]));
    const repairedExisting = (existingMeta || []).map((item) => {
      const latest = latestById.get(String(item?.itemId || ''));
      const accessState = latest?.freeAccessState || item?.freeAccessState;
      return repairEndedFreeDownload(item, accessState);
    });
    const existingById = new Map(repairedExisting.map((item) => [String(item?.itemId || ''), item]));
    const latestWithManual = mergeLatestWithManualItems(repairedExisting, latestMeta).map((item) => {
      if (!item?.isFreeDownload || (Array.isArray(item.downloadLinks) && item.downloadLinks.length > 0)) return item;
      const previous = existingById.get(String(item.itemId || ''));
      const downloadLinks = lastKnownDownloadLinks(previous);
      return downloadLinks.length > 0 ? { ...item, downloadLinks } : item;
    });
    return applyVersionTracking(repairedExisting, latestWithManual || [], detectedAt);
  }
