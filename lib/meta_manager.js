...
function createMetaManager(deps) {
  ...
  function toAssetMap(data) {
    ...
    for (const item of (data || [])) {
      const itemDir = buildItemDir(item.itemId, item.itemName || 'NO_NAME');
  ...
  function normalizeAndPersistMeta(items) {
    ...
    const normalized = dedupeMetaItemsByItemId(items || []);
  ...
  function mergeLatestWithManualItems(existingMeta, latestMeta) {
    ...
    for (let i = 0; i < latest.length; i++) {
      ...
      if (id && existingById.get(id)?.isRemoved) {
        latest[i] = { ...latest[i], isRemoved: false, removedAt: undefined };
      }
    }
    ...
    for (const item of (existingMeta || [])) {
      ...
      if (item?.manualAdded || item?.isWishlisted) {
        ...
        // purchased item disappeared from BOOTH → preserve with isRemoved flag
        latest.push({
          ...item,
          isRemoved: true,
  ...
  async function loadOrGenerateMeta(event, progressScope = 'load-assets') {
    ...
    if (!didStartupNewItemCheck) {
      ...
      startupMetaRefreshPromise = (async () => {
        try {
          const existingIds = (metaCache || []).map((i) => String(i.itemId)).filter(Boolean);
