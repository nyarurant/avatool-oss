...
function registerIpcHandlers(deps) {
  ...
  handleIpc('get-app-edition', async () => ({
    ok: true,
    edition: String(appEdition || 'standard'),
    owner: appEdition === 'owner',
  }));

  if (appEdition === 'owner') {
    require('../owner/ipc_handlers_owner').registerOwnerIpcHandlers({
      handleIpc,
      dialog,
      getMainWindow,
      ownerVaultService,
  ...
  handleIpc('get-agent-integration-status', async () => {
    try {
      return await agentIntegrationService.getStatus();
    } catch (error) {
      return { ok: false, error: error?.message || String(error), agents: {} };
    }
  });

  handleIpc('setup-agent-integration', async () => {
    try {
      const result = await agentIntegrationService.setup({ confirmed: true });
      logMgr.appendOperationLog(
        'agent-integration',
        result?.ok ? 'AIエージェント連携をセットアップしました。' : 'AIエージェント連携のセットアップが一部またはすべて失敗しました。',
      );
      return result;
    } catch (error) {
      return { ok: false, error: error?.message || String(error), agents: {} };
  ...
  handleIpc('demo-simulate-download', async (_event, payload = {}) => {
    if (!demoRecordingService) return { ok: false, error: 'demo_mode_disabled' };
    try {
      return await demoRecordingService.simulateDownload({
  ...
  handleIpc('demo-simulate-unity-import', async () => {
    if (!demoRecordingService) return { ok: false, error: 'demo_mode_disabled' };
    try {
      return await demoRecordingService.simulateUnityImportProgress();
    } catch (error) {
      return { ok: false, error: error?.message || String(error) };
    }
  });

  handleIpc('demo-simulate-update-download', async (_event, payload = {}) => {
    if (!demoRecordingService) return { ok: false, error: 'demo_mode_disabled' };
    try {
      return await demoRecordingService.simulateUpdateDownload({
        itemId: String(payload?.itemId || '').trim(),
        title: String(payload?.title || '').trim(),
        fileName: String(payload?.fileName || '').trim(),
      });
  ...
  handleIpc('get-app-version', async () => ({
    ...
    edition: String(appEdition || 'standard'),
    updaterAvailable: appEdition !== 'owner' && Boolean(electronAutoUpdater),
  }));

  handleIpc('check-app-update', async (_event, payload = {}) => {
    if (appEdition === 'owner') return { ok: false, disabled: true, error: 'owner_update_channel_not_configured' };
  ...
  handleIpc('respond-archive-password', async (_event, payload = {}) => {
    ...
    try { pending.resolve(password); } catch { /* Promise解決失敗は既に処理継続不可のため無視 */ }
    return { ok: true };
  });
      ...
      return await runWithBoothCookieLoginFallback(async () => {
        ...
        if (existing) {
          ...
          if (isWishlisted) {
            ...
            removeItemFromAvatoolWishListName?.(itemId)?.catch?.(() => {});
          }
  ...
  handleIpc('save-settings-profile', async (_event, profileName, profilePayload = null) => {
    ...
    try {
      ...
      // Keep the in-memory profile consistent with disk when the atomic
      // replacement fails. This matters when overwriting an existing profile:
      // deleting it here would lose the last successfully saved value.
  ...
  handleIpc('logout-session', async () => {
    try {
      ...
      if (loginWindow && !loginWindow.isDestroyed()) {
        try { loginWindow.close(); } catch { /* 直後にsetLoginWindow(null)するため失敗は無視 */ }
        setLoginWindow(null);
      }
      ...
      if (fs.existsSync(DEFAULT_SETTINGS.cookieFile)) {
        try { fs.unlinkSync(DEFAULT_SETTINGS.cookieFile); } catch { /* ログアウト処理継続を優先し削除失敗は無視 */ }
      }
      if (fs.existsSync(TEMP_COOKIE_PATH)) {
        try { fs.unlinkSync(TEMP_COOKIE_PATH); } catch { /* ログアウト処理継続を優先し削除失敗は無視 */ }
      }
  ...
  handleIpc('sync-library', async (event, options = {}) => {
    if (demoRecordingService?.isDemoModeEnabled?.()) {
      return demoRecordingService.simulateLibrarySync();
    }
    try {
      return await runWithBoothCookieLoginFallback(async () => {
  ...
  handleIpc('check-updates', async (event) => {
    if (demoRecordingService?.isDemoModeEnabled?.()) {
      return demoRecordingService.simulateCheckUpdates();
    }
    try {
      return await runWithBoothCookieLoginFallback(async () => {
        ...
        const merged = (items || []).map((it) => {
          ...
          return {
            ...
            // `it.downloadLinks` can legitimately be `[]` (e.g. a free item whose
            // JSON fetch failed this round) — `[] || x` always short-circuits to
            // `[]` since an empty array is truthy, silently wiping out a
            // previously-good link list. Fall back on emptiness, not falsiness.
            downloadLinks: (Array.isArray(it.downloadLinks) && it.downloadLinks.length > 0)
              ? it.downloadLinks
              : (prev.downloadLinks || []),
            ...
            freeAccessState: it.freeAccessState || prev.freeAccessState || (prev.isFreeDownload ? 'unknown' : ''),
        ...
        // `updates` only lists hashes that changed in *this* call. An item whose
        // hasUpdate flag is still true from an earlier check (e.g. the user missed
        // the "N件の更新を検出" popup, or it was set by a different sync path) never
        // reappears here since nothing changed again — leaving no way to reopen the
        // popup and actually trigger the forced re-download. Re-surface any item
        // still flagged hasUpdate so "更新チェック" can always offer it again.
        const updateItemIds = new Set((updates || []).map((u) => String(u?.itemId || '')));
        for (const it of normalizedMerged) {
          if (!it?.hasUpdate) continue;
          const id = String(it.itemId || '');
          if (!id || updateItemIds.has(id)) continue;
          updates.push({
            itemId: id,
            itemName: it.itemName || '',
            oldHash: '',
            newHash: it.latestVersion?.filesHashStable || '',
            detectedAt: it.latestVersion?.detectedAt || it.lastChecked || new Date().toISOString(),
          });
          updateItemIds.add(id);
        }
  ...
  // IPC: prepare a 3D model preview — extracts a .unitypackage's mesh/texture/mat
  // assets into a persistent cache (or reports loose .fbx/.obj directly).
  const MAX_MODEL_PREVIEW_FILE_BYTES = 200 * 1024 * 1024;
  const MAX_MODEL_PREVIEW_SIBLING_TEXTURES = 200;
  const MAX_MODEL_PREVIEW_SIBLING_MATS = 100;
  const MODEL_PREVIEW_TEXTURE_EXTS = new Set(['.png', '.jpg', '.jpeg', '.tga']);
  const PREVIEW_CACHE_ROOT_RE = /^__preview_cache\/[^/\\]+$/;
  const MAX_VRC_DATA_BYTES = 32 * 1024 * 1024;

  // Serializing every row just to measure it doubles the cost of a payload that
  // Electron is about to serialize again. Sample instead.
  function estimateSerializedBytes(rows) {
    const list = Array.isArray(rows) ? rows : [];
    if (!list.length) return 0;
    const sampleSize = Math.min(list.length, 50);
    const step = Math.max(1, Math.floor(list.length / sampleSize));
    let sampled = 0;
    let bytes = 0;
    for (let i = 0; i < list.length && sampled < sampleSize; i += step) {
      try {
        bytes += Buffer.byteLength(JSON.stringify(list[i]) || '');
      } catch {
        // A row that cannot be serialized contributes nothing to the estimate.
      }
      sampled += 1;
    }
    if (!sampled) return 0;
    return Math.round((bytes / sampled) * list.length);
  }

  function listSiblingTexturesForLooseMesh(meshFullPath, extractedRoot) {
    const dir = path.dirname(meshFullPath);
    const out = [];
    let entries = [];
    try {
    ...
    for (const ent of entries) {
      if (out.length >= MAX_MODEL_PREVIEW_SIBLING_TEXTURES) break;
      if (!ent.isFile()) continue;
      const ext = path.extname(ent.name).toLowerCase();
      if (!MODEL_PREVIEW_TEXTURE_EXTS.has(ext)) continue;
      const full = path.join(dir, ent.name);
      out.push({ relPath: path.relative(extractedRoot, full).replace(/\\/g, '/'), ext });
    }
    return out;
  }

  function listSiblingMaterialsForLooseMesh(meshFullPath, extractedRoot) {
    const {
      parseUnityMaterial,
    ...
    // Build a shallow guid map from sibling .meta files (Unity loose-export layout).
    const guidMap = new Map();
    for (const ent of entries) {
      if (!ent.isFile() || !ent.name.endsWith('.meta')) continue;
      try {
        const metaText = fs.readFileSync(path.join(dir, ent.name), 'utf8');
        const guid = parseGuidFromMeta(metaText);
        if (!guid) continue;
        const assetName = ent.name.replace(/\.meta$/i, '');
        guidMap.set(guid, path.relative(extractedRoot, path.join(dir, assetName)).replace(/\\/g, '/'));
      } catch {
        // ignore
      }
    }
    const materials = [];
    for (const ent of entries) {
      if (materials.length >= MAX_MODEL_PREVIEW_SIBLING_MATS) break;
      if (!ent.isFile()) continue;
      if (path.extname(ent.name).toLowerCase() !== '.mat') continue;
      const full = path.join(dir, ent.name);
      try {
        const text = fs.readFileSync(full, 'utf8');
        const relPath = path.relative(extractedRoot, full).replace(/\\/g, '/');
  ...
  handleIpc('prepare-model-preview', async (event, itemId, title, relPath) => {
    try {
      const itemRef = normalizeItemRefInput(itemId, title);
      if (itemRef?.error) return itemRef;
      const itemDir = downloadQueue.buildItemDir(itemRef.itemId, itemRef.title);
      ...
      if (ext === '.unitypackage') {
        const unityModelPreview = require('./unity_model_preview');
        // extractRelevantAssetsFromPackage is async (Worker) so main stays responsive.
        const result = await unityModelPreview.extractRelevantAssetsFromPackage(targetPath, itemDir);
        if (result.error) return { error: result.error };
        if (result.noMeshFound || !result.meshes.length) return { error: 'no_mesh_found' };
        const root = `__preview_cache/${path.basename(result.cacheDir)}`;
        return {
          ok: true,
          root,
          ...
          prefabBindings: result.prefabBindings || [],
          maComponents: result.maComponents || [],
          vrcSummary: result.vrcSummary || { total: 0, counts: {}, unscopedCounts: {}, byPrefab: {} },
          prefabs: result.prefabs || [],
        };
      }

      if (ext === '.fbx' || ext === '.obj') {
        const textures = listSiblingTexturesForLooseMesh(targetPath, extractedRoot);
        const materials = listSiblingMaterialsForLooseMesh(targetPath, extractedRoot);
        return {
          ok: true,
          root: '__extracted',
          meshes: [{ relPath: String(relPath || '').replace(/\\/g, '/'), ext }],
          textures,
          materials,
        };
      }
  ...
  handleIpc('bake-unity-animation-preview', async (event, payload = {}) => {
    try {
      const itemRef = normalizeItemRefInput(payload.itemId, payload.title || '');
      if (itemRef?.error) return itemRef;
      const itemDir = downloadQueue.buildItemDir(itemRef.itemId, itemRef.title);
      const extractedRoot = path.join(itemDir, '__extracted');
      let packagePath;
      try {
        packagePath = safeResolveUnder(extractedRoot, payload.relPath || '');
      } catch {
        return { error: 'invalid_rel_path' };
      }
      if (!fs.existsSync(packagePath) || path.extname(packagePath).toLowerCase() !== '.unitypackage') {
        return { error: 'package_not_found' };
      }
      const settings = settingsMgr?.getSettings?.() || {};
      const { bakeUnityAnimation } = require('./unity_animation_baker');
      const baked = await bakeUnityAnimation({
        settings,
        packagePath,
        prefabPathContains: payload.prefabPathContains,
        clipPathContains: payload.clipPathContains,
        fps: payload.fps,
      });
      if (baked?.pruneWarning) {
        appendRuntimeLog?.('warn', 'preview project cache prune failed', {
          error: baked.pruneWarning,
          packagePath,
        });
      }
      return baked;
  ...
  // Official VRCSDK PhysBone solver runs in an isolated Unity project and streams
  // only bone deltas back to the existing three.js renderer.
  handleIpc('start-unity-physbone-preview', async (event, payload = {}) => {
    try {
      const settings = settingsMgr?.getSettings?.() || {};
      const itemId = payload.itemId;
      const title = payload.title || '';
      const relPath = payload.relPath || '';
      const prefabPathContains = payload.prefabPathContains || payload.prefabLabel || '';
      const prefabName = payload.prefabName || '';
      ...
      if (relPath) {
        try {
          packagePath = safeResolveUnder(extractedRoot, relPath);
        } catch {
          return { error: 'invalid_rel_path' };
        }
        if (!fs.existsSync(packagePath)) return { error: 'package_not_found' };
      }

      const sender = event.sender;
      const { startPhysBoneBridge } = require('./unity_physbone_bridge');
      const started = await startPhysBoneBridge({
        settings,
        packagePath,
        prefabPathContains,
        prefabName,
        fps: payload.fps,
        pseudoWind: payload.pseudoWind !== false,
        windStrength: payload.windStrength,
        windFrequency: payload.windFrequency,
        onFrame: (frame) => {
          if (!sender?.isDestroyed?.()) sender.send('unity-physbone-frame', frame);
        },
        onState: (state) => {
          if (!sender?.isDestroyed?.()) sender.send('unity-physbone-state', state);
        },
      });
      if (started?.pruneWarning) {
        appendRuntimeLog?.('warn', 'preview project cache prune failed', {
          error: started.pruneWarning,
  ...
  handleIpc('stop-unity-physbone-preview', async (event, sessionId = '') => {
    try {
      const { stopPhysBoneBridge } = require('./unity_physbone_bridge');
      return stopPhysBoneBridge(String(sessionId || ''));
    } catch (e) {
      return { error: e.message };
    }
  });

  // IPC: read raw bytes of a mesh/texture file resolved under either __extracted
  // (loose files) or __preview_cache/<pkg> (extracted .unitypackage assets).
  handleIpc('read-model-preview-file', async (event, itemId, title, root, relPath) => {
    try {
      const itemRef = normalizeItemRefInput(itemId, title);
      if (itemRef?.error) return itemRef;
      const itemDir = downloadQueue.buildItemDir(itemRef.itemId, itemRef.title);
      ...
      if (safeRoot !== '__extracted' && !PREVIEW_CACHE_ROOT_RE.test(safeRoot)) {
        return { error: 'invalid_root' };
      }
      const baseDir = path.join(itemDir, safeRoot);
      if (!fs.existsSync(baseDir)) return { error: 'root_not_found' };

      let targetPath;
      try {
        targetPath = safeResolveUnder(baseDir, relPath);
      } catch {
        return { error: 'invalid_rel_path' };
      }
      if (!fs.existsSync(targetPath)) return { error: 'file_not_found' };
      ...
      const stat = fs.statSync(targetPath);
      if (!stat.isFile()) return { error: 'not_a_file' };
      if (stat.size > MAX_MODEL_PREVIEW_FILE_BYTES) return { error: 'file_too_large' };
  ...
  // VRCSDK data can be several megabytes on a full avatar. Keep it out of the
  // initial preview response and load only the selected Prefab plus shared
  // expression/Animator assets when the user opens the Expressions panel.
  handleIpc('read-model-preview-vrc-data', async (event, itemId, title, root, prefabRelPath) => {
    try {
      const itemRef = normalizeItemRefInput(itemId, title);
      if (itemRef?.error) return itemRef;
      const safeRoot = String(root || '');
      if (!PREVIEW_CACHE_ROOT_RE.test(safeRoot)) return { error: 'invalid_root' };

      const itemDir = downloadQueue.buildItemDir(itemRef.itemId, itemRef.title);
      const baseDir = path.join(itemDir, safeRoot);
      if (!fs.existsSync(baseDir)) return { error: 'root_not_found' };

      const { refreshCachedVrcComponents } = require('./unity_model_preview');
      const all = await refreshCachedVrcComponents(baseDir);
      const wanted = String(prefabRelPath || '').replace(/\\/g, '/').toLowerCase();
      let components = all.filter((row) => {
        const scoped = String(row?.prefabRelPath || '').replace(/\\/g, '/').toLowerCase();
        return !scoped || (wanted && scoped === wanted);
      });
      let fbxMapError = '';
      let fbxMapCoverage = null;
      const needsFbxObjectMap = wanted && components.some((row) => (
        ['physBone', 'physBoneCollider'].includes(String(row?.type || ''))
        && (!row.objectPath || (!row.rootTransformPath && String(row.rootTransformFileId || '0') !== '0'))
      ));
      if (needsFbxObjectMap) {
        const os = require('os');
        const { spawn } = require('child_process');
        const { resolveEditorPath, createUnityProject } = require('./unity_preview_project');
        const { createUnityFbxObjectMapper } = require('./unity_fbx_object_mapper');
        let guidMap = {};
        try {
          guidMap = JSON.parse(fs.readFileSync(path.join(baseDir, '__preview_guid_map.json'), 'utf8'));
        } catch {
          // A missing GUID map leaves the original component rows untouched.
        }
        const mapper = createUnityFbxObjectMapper({
          ...
          resolveEditorPath,
          createUnityProject,
          templatePath: path.join(__dirname, 'unity_templates', 'AvatoolFbxObjectMapper.cs'),
        });
        const mapped = await mapper.ensureMaps({
          cacheDir: baseDir,
          prefabRelPath,
          rows: components,
          guidMap,
        ...
        if (Array.isArray(mapped?.rows)) components = mapped.rows;
        fbxMapError = mapped?.error || '';
        fbxMapCoverage = mapped?.coverage || null;
        if (mapped?.error) {
          appendRuntimeLog?.('warn', 'FBX object path map fallback failed', {
            error: mapped.error,
            logPath: mapped.logPath || '',
            prefabRelPath,
          });
        }
      }
      if (estimateSerializedBytes(components) > MAX_VRC_DATA_BYTES) return { error: 'vrc_data_too_large' };
      return { ok: true, components, fbxMapError, fbxMapCoverage };
    } catch (e) {
      return { error: e.message };
    }
  });
