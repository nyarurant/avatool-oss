...
function registerIpcHandlers(deps) {
  ...
  // IPC: prepare a 3D model preview — extracts a .unitypackage's mesh/texture
  ...
  handleIpc('prepare-model-preview', async (event, itemId, title, relPath) => {
    try {
      ...
      if (ext === '.unitypackage') {
        ...
        const result = unityModelPreview.extractRelevantAssetsFromPackage(targetPath, itemDir);
        ...
        return { ok: true, root, meshes: result.meshes, textures: result.textures || [] };
      }
