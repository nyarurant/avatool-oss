...
function createSettingsManager(deps) {
  ...
  function normalizeSettingsInPlace(target) {
    ...
    t.launchAtLogin = Boolean(t.launchAtLogin);
    t.appUpdateAutoCheckEnabled = t.appUpdateAutoCheckEnabled !== false;
  ...
  function saveSettingsProfiles() {
    try {
      ensureAppDataRootExists();
      const tmpPath = SETTINGS_PROFILES_PATH + '.tmp';
      fs.writeFileSync(tmpPath, JSON.stringify(settingsProfiles || {}, null, 2), 'utf8');
      fs.renameSync(tmpPath, SETTINGS_PROFILES_PATH);
      return { ok: true };
    } catch (e) {
      console.error('Settings profiles save failed:', e?.message || e);
      throw e;
    }
  }
