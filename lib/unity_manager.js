...
function createUnityManager(deps) {
  ...
  function computeProjectFingerprint(projectPath) {
    ...
    try {
      ...
      for (const e of children) {
        ...
        try { fp += `${e.name}:${fs.statSync(path.join(assetsDir, e.name)).mtimeMs};`; } catch {}
      }
    } catch { return null; }
    return fp;
  }
  ...
  async function runUnityBatchImport(projectPath, packagePaths, onProgress = null, options = {}) {
    ...
    if (renameEntries.length > 0) {
      ...
      try { fs.rmSync(renamePlanPath, { force: true }); } catch {}
    }
    try {
    ...
    return await new Promise((resolve) => {
      ...
      const finalize = (result) => {
        ...
        try { if (timeoutTimer) clearTimeout(timeoutTimer); } catch {}
        try { clearInterval(logTimer); } catch {}
        try { flushLogTail(); } catch {}
        try { if (ipcServer) ipcServer.close(); } catch {}
        resolve(result);
      };
