    ...
    for (let page = 1; page <= totalPages; page++) {
      ...
      if (!itemBlocks.length) {
        ...
        $('.l-library-item-thumbnail').each((_idx, thumb) => {
          ...
          const candidate = parents.find((node) => {
            const $node = $(node);
            return $node.find('.l-library-item-thumbnail').length === 1
              && (
                $node.find('div[data-test="downloadable"]').length > 0
                || $node.find('a[href*="/items/"], a[href*=".booth.pm/items/"]').length > 0
              );
          }) || parents.find((node) => $(node).find('.l-library-item-thumbnail').length === 1);
