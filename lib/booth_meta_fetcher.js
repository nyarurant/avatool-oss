  ...
  for (const item of (items || [])) {
    ...
    if (!prev) {
      map.set(itemId, {
        ...
        freeAccessState: item?.isFreeDownload
          ? (item?.freeAccessState || ((item?.downloadLinks || []).length > 0 ? 'available' : 'unknown'))
          : undefined,
    ...
    map.set(itemId, {
      ...
      freeAccessState: (prev?.isFreeDownload || item?.isFreeDownload)
        ? (((!prev?.isFreeDownload && (prev?.downloadLinks || []).length > 0)
            || (!item?.isFreeDownload && (item?.downloadLinks || []).length > 0)
            || prev?.freeAccessState === 'available'
            || item?.freeAccessState === 'available')
          ? 'available'
          : ((prev?.freeAccessState === 'ended' || item?.freeAccessState === 'ended') ? 'ended' : 'unknown'))
        : undefined,
...
async function generateLibraryMeta(onLog, onProgress, options = {}) {
      ...
      $(itemBlocks).each((i, itemBlock) => {
        ...
        if (imageUrl && itemId && (downloadLinks.length > 0 || source.key === 'free_downloads')) {
          allItems.push({
            ...
            freeAccessState: source.key === 'free_downloads'
              ? (downloadLinks.length > 0 ? 'available' : 'unknown')
              : undefined,
  ...
  for (const item of allItems) {
    ...
    try {
      ...
      item.freeAccessState = classifyFreeDownloadAccess(res.data, freeLinks);
  ...
  for (let i = 0; i < allItems.length; i++) {
    ...
    try {
      ...
      if (item.isFreeDownload && (!Array.isArray(item.downloadLinks) || item.downloadLinks.length === 0)) {
        ...
        item.freeAccessState = classifyFreeDownloadAccess(data, freeLinks);
...
function classifyFreeDownloadAccess(payload, downloadLinks = null) {
  const links = Array.isArray(downloadLinks)
    ? downloadLinks
    : extractFreeDownloadLinksFromItemJson(payload);
  if (links.length > 0) return 'available';

  const variations = Array.isArray(payload?.variations) ? payload.variations : [];
  let sawFreePrice = false;
  let sawPaidPrice = false;
  for (const variation of variations) {
    ...
    for (const rawPrice of priceCandidates) {
      if (typeof rawPrice === 'number' && Number.isFinite(rawPrice)) {
        if (rawPrice === 0) sawFreePrice = true;
        else if (rawPrice > 0) sawPaidPrice = true;
        continue;
      }
      if (typeof rawPrice !== 'string' || !rawPrice.trim()) continue;
      if (isFreePriceText(rawPrice)) {
        sawFreePrice = true;
        continue;
      }
      const numeric = Number(rawPrice.replace(/[,\s\\¥￥円]/g, ''));
      if (Number.isFinite(numeric) && numeric > 0) sawPaidPrice = true;
    }
  }

  if (sawFreePrice) return 'available';
  if (sawPaidPrice) return 'ended';
  return 'unknown';
}
