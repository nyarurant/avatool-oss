...
function createAppUpdater(deps) {
  ...
  function isSafeEmergencyArtifactPath(value) {
  ...
  function isValidSha512Base64(value) {
  ...
  function sha512Matches(expected, actual) {
    if (!isValidSha512Base64(expected) || !isValidSha512Base64(actual)) return false;
    const expectedDigest = Buffer.from(String(expected).trim(), 'base64');
    const actualDigest = Buffer.from(String(actual).trim(), 'base64');
  ...
  function parseEmergencyLatestBackupYml(raw) {
    ...
    // The fallback manifest is trusted only for metadata. Never allow it to
    // redirect an installer download to an arbitrary URL.
    if (isSafeEmergencyArtifactPath(out.path)) {
      out.url = `${getAppUpdateBaseUrl()}/${encodeURIComponent(out.path)}`;
    }
    return out;
  }
  ...
  async function fetchEmergencyUpdateInfo() {
    ...
    if (!parsed.version || !isSafeEmergencyArtifactPath(parsed.path) || !parsed.url || !isValidSha512Base64(parsed.sha512)) {
      throw new Error('latest-backup.yml is missing or has invalid secure fields (version/path/sha512)');
    }
    return parsed;
  }
  ...
  async function startEmergencyUpdateDownload() {
    ...
    if (!isSafeEmergencyArtifactPath(info.path) || !isValidSha512Base64(info.sha512)) {
      return { ok: false, error: 'emergency_update_integrity_metadata_invalid' };
    }
    ...
    if (!sha512Matches(info.sha512, got)) {
      try { fs.unlinkSync(outPath); } catch { /* 破損ファイル削除失敗は無視して検証エラーを優先 */ }
  ...
  function setupAppUpdater() {
    ...
    // Update checks may run automatically, but downloading remains an explicit
    // user action through startAppUpdateDownload().
  ...
  async function installAppUpdateNow() {
    if (emergencyUpdateInfo?.downloadedPath && fs.existsSync(emergencyUpdateInfo.downloadedPath)) {
      try {
        ...
        setTimeout(() => {
          try { app.quit(); } catch { /* インストーラー起動後の終了失敗は無視 */ }
        }, 300);
