...
function createDownloadQueue(deps) {
  ...
  async function fetchFreeDownloadLinksForItem(itemId) {
    try {
      ...
      } catch { /* デバッグログ生成の失敗は本処理に影響しないため無視 */ }
  ...
  async function processQueue(senderOverride) {
    ...
    try {
      ...
      emitQueueStatus(senderOverride);
      if (completedThisRun && typeof onQueueSettled === 'function') {
        try { onQueueSettled(); } catch { /* post-download backup scheduling is best-effort */ }
      }
    }
  }
