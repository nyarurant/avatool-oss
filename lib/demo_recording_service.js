...
function createDemoRecordingService({ getMainWindow, fs, path, appDataRoot }) {
  ...
  // more real (user-owned) item straight into librarymeta.json — the same
  ...
  async function simulateLibrarySync() {
    ...
    // Real item (see scripts/demo/demo_data.js#SYNC_NEW_ITEM) the user owns
    // but that isn't part of the static base library — its thumbnail is
    // pre-copied by createDemoData() so it renders correctly the moment
    // this entry is written.
    const newItemId = '4087393';
    if (!items.some((it) => String(it?.itemId || '') === newItemId)) {
      items.unshift({
        ...
        orderDateTime: now,
        imageUrl: '',
        localImagePath: `./cache/${newItemId}.jpg`,
        categories: [{ href: '', text: '3Dモデル', slug: '' }, { href: 'https://booth.pm/ja/browse/3D%E3%83%84%E3%83%BC%E3%83%AB%E3%83%BB%E3%82%B7%E3%82%B9%E3%83%86%E3%83%A0', text: '3Dツール・システム', slug: '3D%E3%83%84%E3%83%BC%E3%83%AB%E3%83%BB%E3%82%B7%E3%82%B9%E3%83%86%E3%83%A0' }],
        ...
        tagNames: ['demo'],
        downloadLinks: [{ downloadableId: `9${newItemId}`, fileName: 'FadeOut_System_Ver2.0+.zip' }],
        versionHistory: [],
  ...
  // Gated the same way: marks the one already-downloaded demo item (the
  // "Nix" avatar, 6481122 — see scripts/demo/demo_data.js) as having a new
  ...
  async function simulateCheckUpdates() {
    ...
    if (target && !target.hasUpdate) {
      ...
      updates.push({
        itemId: updateItemId,
        itemName: target.itemName || '『ニクス -Nix-』オリジナル3Dモデル',
        oldHash: prevStable,
