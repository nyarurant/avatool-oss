...
function createSchedulerService(deps) {
  ...
  // Undo applySchedulerProfileToConcurrency() once the scheduled window ends (or
  // the scheduler is disabled) — otherwise the profile's concurrency value keeps
  // overriding the user's manually-configured setting for the rest of the day.
  function restoreConfiguredConcurrency() {
    const settings = getSettings();
    queueMgr.getQueueState().concurrency = normalizeConcurrency(settings.concurrency);
  }
  ...
  async function maybeRunScheduledDownloads() {
    ...
    if (!settings.downloadSchedulerEnabled) {
      restoreConfiguredConcurrency();
      return;
    }
    const now = new Date();
    const hour = now.getHours();
    if (!isWithinHourWindow(hour, settings.downloadSchedulerStartHour, settings.downloadSchedulerEndHour)) {
      restoreConfiguredConcurrency();
      return;
    }
  ...
  function startAppUpdateAutoCheckTimer() {
    ...
    if (getSettings()?.appUpdateAutoCheckEnabled === false) return;
