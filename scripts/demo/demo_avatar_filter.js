...
// Storyboard: static library (6 items) -> open avatar filter -> select "Nova"
// -> grid narrows to the 3 Nova-compatible items (still visible behind the
...
async function sequence({ evalJs, delay }) {
  ...
  await evalJs(`window.__demoCursor.moveTo('[data-value="Nova"]', 500)`);
  await evalJs(`window.__demoCursor.click('[data-value="Nova"]')`);
